...
def generate_chart(data, chart_type):
    ...
    if chart_type in ['bar', 'line']:
        ...
        if chart_type == 'bar':
            fig = px.bar(data, x=x_col, y=y_col, title=f"{chart_type.title()} Chart: {y_col} by {x_col}")
        else:
            fig = px.line(data, x=x_col, y=y_col, title=f"{chart_type.title()} Chart: {y_col} over {x_col}")

    elif chart_type == 'pie':
        if len(numeric_cols) == 0 or len(non_numeric_cols) == 0:
            raise ValueError("Need at least one numeric and one non-numeric column.")
        fig = px.pie(data, values=numeric_cols[0], names=non_numeric_cols[0], title="Pie Chart")
